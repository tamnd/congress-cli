// Package cli assembles the congress command tree from the congress
...
// NewApp assembles the kit application from the congress domain. The
...
//
// To add a command, declare it in congress/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := congress.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(congress.Domain{}).Register(app)
