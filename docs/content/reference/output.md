...
describes what users get. Pick a format with `-o`, or let congress choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
congress <command> -o table   # aligned columns for reading
congress <command> -o jsonl   # one JSON object per line, for piping
congress <command> -o json    # a single JSON array
congress <command> -o csv     # spreadsheet friendly
congress <command> -o tsv     # tab-separated
congress <command> -o url     # just the URL column
congress <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
congress <command> --fields id,title,url
```
...
```bash
congress <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
congress <command>            # a table, because this is a terminal
congress <command> | wc -l    # JSONL, because this is a pipe
```
